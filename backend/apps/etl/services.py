...
class ETLService:
    ...
    def load(self, df: pd.DataFrame) -> int:
        self._log(f'LOAD: Cargando {len(df)} registros en MySQL...')
        start = time.time()
        loaded = 0
        errors = 0

        with transaction.atomic():
            # Limpiar tabla existente o actualizar
            existing_ids = set(Paciente.objects.values_list('id_paciente', flat=True))
            ...
            for _, row in df.iterrows():
                try:
                    p = self._row_to_paciente(row)
                    if p.id_paciente in existing_ids:
                        pacientes_to_update.append(p)
                    else:
                        pacientes_to_create.append(p)
                except Exception as e:
                    errors += 1
                    self._log(f'LOAD ERROR fila {row.get("id_paciente")}: {e}', 'ERROR')

            # Bulk create
            if pacientes_to_create:
                Paciente.objects.bulk_create(pacientes_to_create, batch_size=200, ignore_conflicts=True)
                loaded += len(pacientes_to_create)

            # Update existentes
            for p in pacientes_to_update:
                try:
                    Paciente.objects.filter(id_paciente=p.id_paciente).update(
                        nombres=p.nombres, apellidos=p.apellidos, edad=p.edad,
                        sexo=p.sexo, peso=p.peso, altura=p.altura,
                        imc=p.imc, imc_clasificacion=p.imc_clasificacion,
                        ...
                        fumador=p.fumador, consumo_alcohol=p.consumo_alcohol,
                        ...
                        es_hipertenso=p.es_hipertenso,
                        es_diabetico=p.es_diabetico,
                    )
                    loaded += 1
                except Exception as e:
                    errors += 1
        ...
        self._log(f'LOAD: {loaded} registros cargados, {errors} errores en {elapsed}s')
        return loaded
